...
public class ForestLandDetailsController {
	...
	/*
	 * @RequestMapping(value = "/MapView", method = RequestMethod.GET) public String
	 * viewFLDMap(Model model, @RequestParam String layer, @RequestParam Integer
	 * distid,
	 * 
	 * @RequestParam Integer divid, @RequestParam String rangeCode) throws
	 * JsonProcessingException { try { List<SP_NFBViewModel> objList =
	 * frlservice.getNFBLayers(layer, distid, divid, rangeCode); ObjectMapper mapper
	 * = new ObjectMapper(); model.addAttribute("layer",
	 * mapper.writeValueAsString(objList)); model.addAttribute("village_code", "");
	 * return "/fld/MapView"; } catch (Exception ex) { throw ex; } }
	 */
