...
//	}
	...
	public ResponseEntity<?> projectDeleteFunc(@PathVariable String intId, Model model, HttpSession session) {
		//userProjectMasterRepository.deleteById(Integer.parseInt(intId));
		 UserProjectMaster userProjectMaster = userProjectMasterRepository.findById(Integer.parseInt(intId)).get();
		//UserProjectMaster project=new UserProjectMaster();
		if(userProjectMaster.getDeletedStatus()!=0)
		userProjectMaster.setDeletedStatus(0);
		userProjectMasterRepository.save(userProjectMaster);
		//setSession(model, session);
		//session.setAttribute("userProjList",List.of( userProjectMaster).stream().filter(f->(f.getDeletedStatus()!=null?f.getDeletedStatus():0)==1).collect(Collectors.toList()));
		List<UserProjectMaster> findAllByUser = userProjectMasterRepository.findAllByUser(autoUserInfoRepository.findById(Integer.valueOf(session.getAttribute("PkUserId").toString())).get());
		List<UserProjectMaster> collect = findAllByUser.stream().filter(f->(f.getDeletedStatus()!=null?f.getDeletedStatus():0)==1).collect(Collectors.toList());
		session.setAttribute("userProjList", collect);
