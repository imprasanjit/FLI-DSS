		...
		if (this.checked) {
			...
			$.ajax({
				...
				success: function(data) {
					map.removeLayer(divlayer);//for remove layer after select another layer
...
//District boundary Checkbox Change Event//-----------------------------------------------------------------//
...
//-----------------------------------------------------------------//
var layerList = [];
function zoomLayer(url, zoom, layer) {
	try {
		//alert(layer)
		for (var i = 0; layerList.length > 0; i++) {
			//alert(layerList[i].get("lname"))
			if (layerList[i].get("lname") === layer) {
